"""
preprocess_data_hard_neg.py - Convert local raw datasets into JSONL format
                              with BM25 hard negative mining.

Extends preprocess_data.py by replacing pure random negatives with a
mixed strategy: n_hard BM25 top-k negatives + remaining random negatives.
...
Usage
-----
# SciFact with 2 hard + 2 random negatives (default)
python preprocess_data_hard_neg.py --dataset scifact \\
    --input_dir "../data/raw/scifact" \\
    --output_dir ../data/scifact_hard

# SciFact with all-hard negatives
python preprocess_data_hard_neg.py --dataset scifact \\
    --input_dir "../data/raw/scifact" \\
    --output_dir ../data/scifact_allhard \\
    --negatives_per_positive 4 --n_hard 4

# SciDocs with hard negatives
python preprocess_data_hard_neg.py --dataset scidocs \\
    --input_dir "../data/raw/scidocs" \\
    --output_dir ../data/scidocs_hard

Ablation comparison
-------------------
Train on ../data/scifact/      (random)  -> outputs/scifact_random
Train on ../data/scifact_hard/ (hard)    -> outputs/scifact_hard
Compare ndcg_at_10 / mrr on the same test.jsonl.

Note: neg_type field ("hard" / "random" / "positive") is written to JSONL
for analysis only. train.py ignores it.
...
def _build_scifact(args) -> tuple[list, list, list]:
    ...
    train_all = _make_pairs_with_hard_neg(
        ...
        bm25_index, corpus_ids, n_hard=args.n_hard,
    )
    train_rows, val_rows = split_by_query_id(train_all, args.validation_ratio, args.seed)

    # Test set: use plain random negatives so evaluation is unbiased
    # (the model is evaluated on candidates it hasn't been specifically
    #  trained to discriminate from BM25-retrieved hard examples)
...
def _build_scidocs(args) -> tuple[list, list, list]:
    ...
    # SciDocs only has a test split — split it into train/val/test
    test_qrels = _load_qrels(scidocs_dir / "qrels" / "test.tsv")

    bm25_index, corpus_ids = _build_bm25_index(corpus)

    all_rows = _make_pairs_with_hard_neg(
        queries, corpus, test_qrels,
        args.negatives_per_positive, rng,
        bm25_index, corpus_ids, n_hard=args.n_hard,
    )
    ...
    # Adjusted validation ratio relative to remaining train_val pool
